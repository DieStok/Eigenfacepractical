...
p_load(ggplot2, magrittr, plyr, tidyverse, EBImage, stringr, devtools, R.matlab, gganimate, png, gifski, knitr, ggdendro, Rtsne, uwot)
...
##########################################
#
#       K-means clustering and PCA start
#
##########################################
...
That's all for k-means, one of the most well-known types of clustering using the prototype method. However, another large branch of clustering is hierarchical clustering. We use it in phylogeny, for instance. Let's use it on the initial data we generated all the way at the top and see what we get. If you want a nice beginner's guide for hierarchical clustering, look here: https://www.datacamp.com/community/tutorials/hierarchical-clustering-R 
...
Q6: What final step do you need to take to get clusters, based on what you want out of your clustering?
...
That's fine and dandy, but now we'd like to use these clusters. So, how do you cut the dendrogram and use the resultant clusters? Using cutree() on the objects you got from running the hclust() function. Up to you to use cutree to get 4 clusters that were clustered using average linkage and plotting them. Then, try 8 clusters and plot those. Finally, try it on single linkage data with 4 clusters.
...
#Answer: One cluster is one point, which is quite hilarious. It happens because this point is so far from the other in the bottom right cluster that it only joins them as the last point. Meanwhile, the left two clusters have a sort of bridge between them that apparently allow them to be more easily combined.
...
plotMNISTPCA = ggbiplot::ggbiplot(digitsPCA, var.axes = FALSE, alpha = 0.3,
...
Let's move on to t-SNE. Use the Rtsne() function on the MNIST data and plot the outcome (with the correct labels). I'm leaving you to it without further instruction, since you've seen plotting code before and it's not so different from plotting the PCA results. You can look here: https://www.rdocumentation.org/packages/Rtsne/versions/0.15 and just use ?Rtsne for help. Be sure to try different perplexities (See here: https://distill.pub/2016/misread-tsne/). You can use ggplot to plot, but R's built-in plot function is also fine.
...
Q3: Is there any sneaky PCA stuff going on even though we're running t-SNE?
...
Finally, let's have a look at UMAP. I trust you know the drill. Run UMAP and plot the results. Be sure to read a little bit of https://umap-learn.readthedocs.io/en/latest/ to understand the basic parameters. The function call is umap()
