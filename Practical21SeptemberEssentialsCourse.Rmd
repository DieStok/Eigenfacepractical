...
p_load(ggplot2, magrittr, plyr, tidyverse, EBImage, stringr, devtools, R.matlab, gganimate, png, gifski)
...
#just the 2D data, showing that PCA does nothing if you keep the initialdata
pca2DData = prcomp(dataforClustering %>% select(-"True Cluster"), center = TRUE, scale. = TRUE)

#plot it
ggbiplot::ggbiplot(pca2DData, groups = dataWithNoise$`True Cluster`, varname.adjust = 1.1, varname.abbrev = TRUE) + theme_bw()
...
Now, let's run kmeans on the multidimensional data, and then show the groups found in the 2D space. Note that R's k-means function automatically does multiple initialisations and chooses the 'best' clusters (it is smarter than the basic k-means explained in the lectures). Below is the example for k = 4. 
...
Now let's add more noisy dimensions and see what happens with clustering. We'll take our data from above, and just start adding random noisy genes. Then we'll do PCA and visualise the first 2 components each time. 
