...
```{r}
centersForRuns = vector("list", 4)

for( i in seq_along(centersForRuns)) {
  centersForRuns[[i]] = dataforClustering[sample(seq(1:nrow(dataforClustering)), 4), ]
  }
centersForRuns 

##KMEANS IN R IS SMARTER THAN WHAT YOU LEARN AS STANDARD DESCRIPTION!


#implement own k-means function
data = dataforClustering[, 1:2]; startCentroids = centersForRuns[[1]][, 1:2]
k = 4
iterations = 10
distance = "euclidean"
#doKMeans = function(data, startCentroids, k = 4, iterations = 10, distance = "euclidean") {
  centroidList = vector("list", iterations)
  clusterList  = vector("list", iterations)
  ...
  for (i in seq(1:iterations)) {
    ...
    for (clusterKDistance in seq(1:k)) { 
      ...
      if (distance == 'euclidean') {
        ...
        distVec = (data - currentCentroids[clusterKDistance, ])^2
    ...
    m = bind_cols(distancesList)
    break
    
    
    
    newCentroids = data %>% group_by(currentCluster) %>% mean()
...
firstClusterPlot = function(data, centers) {
  
dfCluster <- kmeans(data[, 1:2],centers=centers[, 1:2], iter.max = 1, algorithm = "MacQueen")
centersForPlotting = dfCluster$centers %>% as_tibble() %>% add_column(centroid = as.factor(1:nrow(dfCluster$centers)))
dataforClustering$kMeansClusterIteration1 = as.character(dfCluster$cluster)
firstIterationClusteringPlot = ggplot(data = data,
       aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`", colour = "kMeansClusterIteration1")) +
  geom_point(size = 3) +
  geom_point(size = 5, stroke = 2.5, shape = 21, data = centersForPlotting, colour = "black",
             aes_string(fill = "centroid"),
             show.legend = NA) +
  theme_bw()
  firstIterationClusteringPlot
  
  
}
iterationPlot = function(dataPoints, dataCenters, iteration) {
  itPlot = ggplot(data = dataPoints,
       aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`",
                  colour = paste0("kMeansClusterIteration", iteration)
                  )) +
  geom_point(size = 3) +
  geom_point(size = 5, stroke = 2.5, shape = 21, data = dataCenters, colour = "black",
             aes_string(fill = "centroid"),
             show.legend = NA) +
  theme_bw()
  itPlot
...
beginPlot = preClusterPlot(data = dataforClustering, centers = centersForRuns[[1]])
itOnePlot = firstClusterPlot(data = dataforClustering, centers = centersForRuns[[1]])
show(beginPlot)
show(itOnePlot)
...
max_iter = 10
itPlotList = vector("list", length = max_iter)

for (i in 2:max_iter){
  tryCatch({
    dfCluster <- kmeans(dataforClustering[, 1:2], centers = dfCluster$centers, iter.max = 1, algorithm = "MacQueen")
    done <- TRUE
  }, 
  warning=function(w) {done <- FALSE})
  cent[[i]] <- dfCluster$centers %>% as_tibble() %>% add_column(centroid = as.factor(1:nrow(dfCluster$centers)))
  dataforClustering[[paste0("kMeansClusterIteration", i)]] = as.character(dfCluster$cluster)
  itPlotList[[i]] = iterationPlot(dataforClustering, cent[[i]], i)
  if(done) break
}

totalPlotListThisRun = list(beginPlot, itOnePlot)
totalPlotListThisRun[3:(length(itPlotList)+3)] = itPlotList
...
```{r}
#non-functionwrapped older code:
  
  #beginning situation
beforeClustering = ggplot(data = dataforClustering, aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`", colour = "`True Cluster`")) +
  geom_point(size = 3) +
  geom_point(size = 5, stroke = 2.5, shape = 21, data = centersForRuns[[1]], colour = "black",
             aes_string(fill = "`True Cluster`"),
             show.legend = FALSE) +
  scale_fill_discrete(drop = FALSE) +
  theme_bw()  
show(beforeClustering)

#iteration 1 of Kmeans
dfCluster <- kmeans(dataforClustering[, 1:2],centers=centersForRuns[[1]][, 1:2], iter.max = 1)
centersForPlotting = dfCluster$centers %>% as_tibble() %>% add_column(centroid = as.factor(1:nrow(dfCluster$centers)))
dataforClustering$kMeansClusterIteration1 = as.character(dfCluster$cluster)
firstIterationClustering = ggplot(data = dataforClustering,
       aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`", colour = "kMeansClusterIteration1")) +
  geom_point(size = 3) +
  geom_point(size = 5, stroke = 2.5, shape = 21, data = centersForPlotting, colour = "black",
             aes_string(fill = "centroid"),
...
Q7: Do you think that this n*n-dimensional space is evenly (or randomly) filled with examples from this data, or not? Why? #PERHAPS MAKE HERE A RANDOM GRAYSCALE IMAGE. 
