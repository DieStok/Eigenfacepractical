...
We'll start by performing some K-means clustering. The code block below generates some random data with which to perform the clustering. We colour the points according to the pre-set cluster they are supposed to belong to.
...
ggplot(data = dataforClustering, aes_string(x = "`Gene 1 Expression`", y = "`Gene 2 Expression`", colour = "Cluster")) + geom_point(size = 3) +
  theme_bw() +
  
  

```
Now it is time to perform some clustering on this data. Run the code block below, and answer the questions.
...
for( i in seq_along(centersForRuns)) {
  centersForRuns[[i]] = dataforClustering[sample(seq(1:nrow(dataforClustering)), 4), 1:2]
  kMeans
  }
centersForRuns 


kmeans(dataforClustering[, 1:2], 4)
```
