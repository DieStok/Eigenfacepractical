...
dfCluster <- kmeans(data[, 1:2],centers=centersForRuns[[1]][, 1:2], iter.max = 1)
centersForPlotting = dfCluster$centers %>% as_tibble() %>% add_column(centroid = as.factor(1:nrow(dfCluster$centers)))
data$kMeansClusterIteration1 = as.character(dfCluster$cluster)
firstIterationClusteringPlot = ggplot(data = data,
  ...
  geom_point(size = 5, stroke = 2.5, shape = 21, data = centersForPlotting, colour = "black",
             ...
             show.legend = NA) +
  theme_bw()
  list(data = data, plot = firstIterationClusteringPlot)
...
for (i in 2:max_iter){
  tryCatch({
    dfCluster <- kmeans(dataforClustering[, 1:2], centers = dfCluster$centers, iter.max = 1)
    done <- TRUE
  }, 
